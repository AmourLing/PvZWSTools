            ...
            _connection.ConnectionStateChanged += (s, connected) =>
            {
                ...
                if(connected)
                {
                    if(!SuppressConnectionMessage)
                        _ = _connection.SendAsync(Sharedstring.GetLogoDisplayString);
                    Log.Info($"已成功连接到{WsAddress}");
                }
            };

            _connection.ConnectionError += (s, error) =>
            {
                _ = MessageBox.Show($"连接错误：{error}", "WebSocket", MessageBoxButton.OK, MessageBoxImage.Warning);
                Log.Error(error);
            };
