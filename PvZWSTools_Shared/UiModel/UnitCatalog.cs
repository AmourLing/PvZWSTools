...
public static class UnitCatalog
{
    ...
    private static UnitDescriptor Grp(string label, string cmd, UnitField[] fields,
        ...
        Grp(label, cmd, fields, Array.Empty<string>(), members);

    /// <summary>同上，再给组本身补一组搜索别名：收成组以后原来挂在选项上的英文别名
    /// （大蒜/黄油/冰封在界面上只有中文下拉项可认）就没有落脚处了。</summary>
    private static UnitDescriptor Grp(string label, string cmd, UnitField[] fields,
                                      string[] kw, params UnitDescriptor[] members) =>
        new()
        {
            Id = cmd,
            Label = Loc.T(label),
    ...
    public static IReadOnlyList<NavItem> Build(object root)
    {
        var pages = new List<NavItem>
        {
            ...
            Page("僵尸", "\uE7C2",
                ...
                Grp("施加僵尸状态", z + "ApplyZombieStateCommand",
                    [F("状态", z + "ZombieStateInput", z + "ZombieStateOptions",
                       z + "SelectedZombieState", 190)],
            ...
            Page("出怪", "\uE81D",
                // 同步出怪列表发布的就是这 40 个勾的当前值（游戏侧推给 UpdateZombieStatesFromOutput），
                // 它是这一块的数据来源，不是另一件事
                Grp(
                ...
                Sw("暂停出怪", s + "StopSpawn"),
                Sw("最大密度", s + "MaxPoint"),
                Sw("飞贼处理", s + "BungeeCheck", s + "BungeeHandleCommand"),
