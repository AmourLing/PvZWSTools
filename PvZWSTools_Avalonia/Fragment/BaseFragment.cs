...
public abstract class BaseFragment:AndroidX.Fragment.App.Fragment
{
    ...
    private void LoadOptionsInternal()
    {
        try
        {
            // 复用 MainActivity 已确定的静态应用文件路径，避免重复查询外部存储目录
            if(string.IsNullOrEmpty(MainActivity.AppFilesPath)) return;

            var configPath = Path.Combine(MainActivity.AppFilesPath, "配置文件", "选项");
