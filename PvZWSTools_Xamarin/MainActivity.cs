...
namespace PvZWSTools_Xamarin
{
    ...
    {
        public static readonly string mDefaultPath = "/storage/emulated/0/Documents/PvZWSTools";
        ...
        private void ShowSettingsDialog()
        {
            ...
            layout.SetPadding(50, 30, 50, 30); // 设置内边距
            ...
            chkAutoConnect.SetPadding(0, 0, 0, 30); // 底部边距
            ...
            chkShowNotification.SetPadding(0, 0, 0, 10); // 底部边距
            ...
            builder.SetPositiveButton("确定", (sender, e) =>
            {
                ...
                {
                    // 如果AppSettings类中确实有ReconnectInterval属性
        ...
        protected override void OnCreate(Bundle savedInstanceState)
        {
            ...
            try
            {
                if(Directory.Exists(mDefaultPath))
                {
                    LogHelper.Initialize(mDefaultPath);
                    LogHelper.Log("MainActivity 启动");
                }
                else
                {
                    // 尝试创建目录
                    Directory.CreateDirectory(mDefaultPath);
                    LogHelper.Initialize(mDefaultPath);
                    LogHelper.Log("创建了默认目录并初始化日志");
                }
            }
            catch(Exception logEx)
            {
                Android.Util.Log.Error("MainActivity", $"日志初始化失败: {logEx.Message}");
            }
            ...
            // 初始化设置
            _settingsPath = Path.Combine(mDefaultPath, "配置文件", "setting.json");
