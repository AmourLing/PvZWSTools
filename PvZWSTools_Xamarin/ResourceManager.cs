...
namespace PvZWSTools_Xamarin
{
    public static class ResourceManager
    {
        ...
        // 添加一个事件用于报告解压进度
        public static event EventHandler<ResourceExtractProgressEventArgs> ExtractProgress;
        ...
        public static bool CheckResourceIntegrity()
        {
            ...
            try
            {
                ...
                foreach(var dir in requiredDirs)
                {
                    ...
                    {
                        Log.Warn("ResourceManager", $"资源完整性检查失败: {dir} 目录不存在");
                        return false;
                    }
                    var files = Directory.GetFiles(fullPath, "*", SearchOption.AllDirectories);
                    if(files.Length == 0)
                    {
                        Log.Warn("ResourceManager", $"资源完整性检查警告: {dir} 目录为空");
                    }
                }
                Log.Info("ResourceManager", "资源完整性检查通过");
                return true;
            }
            catch(Exception ex)
            {
                Log.Error("ResourceManager", $"资源完整性检查异常: {ex}");
                return false;
            }
        }
            ...
            lock(_lockObject)
            {
                try
                {
                    if(Directory.Exists(_externalResourcesPath))
                    {
                        ...
                        Log.Info("ResourceManager", "已删除旧资源目录");
                    }

                    _isInitialized = false;
                    Initialize();

                    Log.Info("ResourceManager", "资源强制重新解压完成");
                }
                catch(Exception ex)
                {
                    Log.Error("ResourceManager", $"强制重新解压资源失败: {ex}");
                    throw;
                }
            }
        }
        ...
        public static void Initialize()
        {
            ...
            lock(_lockObject)
            {
                ...
                try
                {
                    ...
                    Log.Info("ResourceManager", "开始初始化资源管理器...");
                    CheckAndExtractResources();
                    _isInitialized = true;
                    Log.Info("ResourceManager", $"资源初始化完成，路径: {_externalResourcesPath}");
                }
                catch(Exception ex)
                {
                    Log.Error("ResourceManager", $"资源初始化失败: {ex}");
                    throw;
                }
            }
        }
        ...
        public static void MarkForUpdate()
        {
            try
            {
                ...
                if(File.Exists(_versionFilePath))
                {
                    ...
                    Log.Info("ResourceManager", "已标记需要更新资源");
                }
                ...
                Log.Info("ResourceManager", "已创建强制更新标记文件");
            }
            catch(Exception ex)
            {
                Log.Error("ResourceManager", $"标记更新失败: {ex}");
            }
        }
        ...
        public static bool NeedsUpdate()
        {
            try
            {
                ...
                if(!Directory.Exists(_externalResourcesPath))
                {
                    Log.Info("ResourceManager", "NeedsUpdate: 资源目录不存在");
                    return true;
                }
                ...
                if(!File.Exists(_versionFilePath))
                {
                    Log.Info("ResourceManager", "NeedsUpdate: 版本文件不存在");
                    return true;
                }
                ...
                if(savedInfo == null)
                {
                    Log.Info("ResourceManager", "NeedsUpdate: 无法读取资源信息");
                    return true;
                }
                ...
                Log.Info("ResourceManager", $"NeedsUpdate检查结果: " +
                    $"版本变化={versionChanged}, " +
                    $"ZIP变化={zipChanged}, " +
            ...
            {
                Log.Error("ResourceManager", $"NeedsUpdate检查失败: {ex}");
                // 出现异常时，建议更新更安全
                return true;
            }
        }
        ...
        private static bool AreResourceDirectoriesValid()
        {
            try
            {
                ...
                foreach(var dir in requiredDirs)
                {
                    ...
                    if(!Directory.Exists(dirPath))
                    {
                        Log.Warn("ResourceManager", $"资源目录验证失败: {dir} 目录不存在");
                        return false;
                    }
                    ...
                    if(files.Length == 0)
                    {
                        Log.Warn("ResourceManager", $"资源目录验证警告: {dir} 目录为空");
        ...
        private static void CheckAndExtractResources()
        {
            try
            {
                ...
                if(needsExtraction)
                {
                    Log.Info("ResourceManager", "开始解压资源文件...");

                    Directory.CreateDirectory(_externalResourcesPath);
                    ExtractZipFromAssets(_assetsZipPath, _externalResourcesPath);

                    // 保存版本信息和ZIP文件哈希值
                    SaveResourceInfo(context);

                    Log.Info("ResourceManager", "资源解压完成");
                    VerifyExtraction();
                }
                else
                {
                    Log.Info("ResourceManager", "资源已是最新，跳过解压");
                }
            }
            catch(Exception ex)
            {
                Log.Error("ResourceManager", $"资源检查失败: {ex}");
                throw;
            }
        }
        ...
        private static void ExtractZipFromAssets(string zipFileName, string extractPath)
        {
            ...
            using(var assetStream = context.Assets.Open(zipFileName))
            {
                ...
                Log.Info("ResourceManager", "开始解压ZIP文件...");
                ...
                while((entry = zipInputStream.NextEntry) != null)
                {
                    try
                    {
                        ...
                        if(entry.IsDirectory || entryName.EndsWith("/"))
                        {
                            ...
                            Log.Debug("ResourceManager", $"创建目录: {dirPath}");
                            continue;
                        }
                        ...
                        if(fileCount % 10 == 0)
                        {
                            Log.Info("ResourceManager", $"已解压 {fileCount} 个文件: {entryName}");
                        }
                    ...
                    {
                        Log.Error("ResourceManager", $"解压文件 {entry?.Name} 失败: {ex}");
                    }
                }

                zipInputStream.Close();
                Log.Info("ResourceManager", $"解压完成，总共解压了 {fileCount} 个文件");
            }
        }
        ...
        private static bool IsForceUpdateNeeded(ResourceInfo savedInfo)
        {
            try
            {
                ...
                {
                    Log.Info("ResourceManager", $"调试模式：距离上次更新已超过24小时，强制更新");
                    return true;
                }
                ...
                {
                    Log.Info("ResourceManager", "找到强制更新标记文件，强制更新");
        ...
        private static void SaveResourceInfo(Context context)
        {
            try
            {
                ...
                Log.Info("ResourceManager", $"资源信息已保存: {info.ApkVersion}, Hash={currentZipHash.Substring(0, 16)}...");
            }
            catch(Exception ex)
            {
                Log.Error("ResourceManager", $"保存资源信息失败: {ex}");
            }
        }
        ...
        private static bool ShouldExtractResources(Context context)
        {
            try
            {
                ...
                if(!Directory.Exists(_externalResourcesPath))
                {
                    Log.Info("ResourceManager", "资源目录不存在，需要解压");
                    return true;
                }
                ...
                if(!File.Exists(_versionFilePath))
                {
                    Log.Info("ResourceManager", "版本文件不存在，需要解压");
                    return true;
                }
                ...
                if(savedInfo == null)
                {
                    Log.Info("ResourceManager", "无法读取资源信息，需要解压");
                    return true;
                }
                ...
                if(needsExtraction)
                {
                    Log.Info("ResourceManager", $"需要解压资源: " +
                        $"版本变化={versionChanged}, " +
                        $"ZIP变化={zipChanged}, " +
                        $"目录有效={directoriesValid}, " +
                        $"强制更新={forceUpdateNeeded}");
            ...
            {
                Log.Error("ResourceManager", $"检查资源状态失败: {ex}");
                // 出现异常时，重新解压更安全
                return true;
            }
        }
        ...
        private static bool ShouldExtractResources(string currentVersion)
        {
            try
            {
                ...
                {
                    Log.Info("ResourceManager", $"版本号变化 ({savedVersion} -> {currentVersion})，需要解压");
                    return true;
                }
                ...
                foreach(var dir in requiredDirs)
                {
                    ...
                    if(!Directory.Exists(dirPath))
                    {
                        Log.Info("ResourceManager", $"目录 {dir} 不存在，需要解压");
                        return true;
                    }
                }
        ...
        private static void VerifyExtraction()
        {
            try
            {
                Log.Info("ResourceManager", "验证解压结果...");
                ...
                foreach(var dir in directories)
                {
                    ...
                    {
                        var files = Directory.GetFiles(dirPath, "*", SearchOption.AllDirectories);
                        Log.Info("ResourceManager", $"  - {dir}: {files.Length} 个文件");
                        foreach(var file in files.Take(5))
                        {
                            var fileInfo = new FileInfo(file);
                            var relativePath = file.Substring(_externalResourcesPath.Length + 1);
                            Log.Debug("ResourceManager", $"    - {relativePath} ({fileInfo.Length} bytes)");
                        }
                    }
                    else
                    {
                        Log.Warn("ResourceManager", $"  - {dir}: 目录不存在!");
                    }
                }
                var allFiles = Directory.GetFiles(_externalResourcesPath, "*", SearchOption.AllDirectories);
                Log.Info("ResourceManager", $"解压完成，总共 {allFiles.Length} 个文件");
            }
            catch(Exception ex)
            {
                Log.Error("ResourceManager", $"验证解压结果失败: {ex}");
            }
        }
